    ...
    public partial class frmSendMessages : Form
    {
        ...
        private async void btnOK_Click(object sender, EventArgs e)
        {
            ...
            for (int j = totalMessagesCount; j >= 0; j -= takeCount)
            {
                ...
                var history = await frmMain.tgClient.Messages_GetHistory(sourceChat, add_offset: offset, limit: limitCount);
                    ...
                    if (pm.GetType() == typeof(TL.Message))
                    {
                        ...
                        if (msg.grouped_id == 0)
                        {
                            if (msg.media == null)
                            {
                                ...
                                {
                                    await frmMain.tgClient.SendMessageAsync(peer: targetChat, text: text);
                                }
                            }
                            else
                                await frmMain.tgClient.SendMessageAsync(peer: targetChat, text: text, media: msg.media.ToInputMedia(), entities: msg.entities);
                        }
                        else
                        {
                            if (msg.grouped_id != lastMediAGroupId)
                            {
                                ...
                                var hst = await frmMain.tgClient.Messages_GetHistory(targetChat);
                                ...
                                await frmMain.tgClient.SendAlbumAsync(peer: targetChat, medias: albumGroupMedia, caption: text);
                            }
                        }
                    }
                    else
